    ...
    run: async ({ client, message }) => {
        ...
        GuildForceSkip.findOne({
            ...
            if (resultData) {
                ...
                message.channel.createMessage({ content: `${client.emote.ok} Track repeat has been ${player.trackRepeat ? "**enabled**" : "**disabled**"}.` });
            } else {
                const botVoice = await client.getRESTGuildMember(message.guildID, client.user.id);
                ...
                if (!player || !player.queue.current) return message.channel.createMessage({ content: `${client.emote.error} Not playing anything in voice channel.` });
                player.setTrackRepeat(!player.trackRepeat);
                return message.channel.createMessage({ content: `${client.emote.ok} Track repeat has been ${player.trackRepeat ? "**enabled**" : "**disabled**"}.` });
            }
            return null;
        });
        return null;

module.exports = {
    ...
    run: async ({ client, message }) => {
        ...
        if (player.pop) {
            message.channel.createMessage({ content: `${client.emote.removingFilter} Removing pop filter, please wait...` });
            player.pop = !player.pop;
        } else {
            message.channel.createMessage({ content: `${client.emote.ok} Activating pop filter, please wait...` });
            player.pop = !player.pop;
        }
        return null;
    },

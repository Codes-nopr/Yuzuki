module.exports = {
    ...
    run: async ({ client, message }) => {
        ...
        if (player.nightcore) {
            message.channel.createMessage({ content: `${client.emote.removingFilter} Removing nightcore filter, please wait...` });
            player.nightcore = !player.nightcore;
        } else {
            message.channel.createMessage({ content: `${client.emote.ok} Activating nightcore filter, please wait...` });
            player.nightcore = !player.nightcore;
        }
        return null;
    },
